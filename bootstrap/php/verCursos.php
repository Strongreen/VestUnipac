	...
	<section id="contato">
		<div class="container">
			<div class="row">
				...
				<h4 class="text-muted">Lista de Cursoss.</h4>
				</div>
			</div>
			<br>
		</div>
...
while ($login = $resultado->fetch_object()) {
  // Exibe a notícia dentro de um bloco HTML
	...
	echo 	'<tr>
				...
				<td style="white-space: pre-wrap; text-align:justify">' .$login->resumo. '</td>
			</tr>';
	echo 	'<tr>
				<td><label>Autorização:</label></td>
				<td>' .$login->autorizacao. '</td>
			</tr>';
	echo 	'<tr>
	...
	echo 	'<tr>
				...
				<td>' .$login->periodos. '</td>
			</tr>';
	echo 	'<tr>
				<td><label>Objetivo:</label></td>
				<td style="white-space: pre-wrap; text-align:justify">' .$login->objetivo. '</td>
			</tr>';
	echo 	'<tr>
				<td><label>Coordenação:</label></td>
				<td style="white-space: pre-wrap; text-align:justify">' .$login->coordenacao. '</td>
			</tr>';
} 
	echo '</tbody></table>';	
	echo '<a class="btn btn-sm btn-primary col-lg-12 " align="center" href="modificarCursos.php?id='.$_REQUEST['id'].'">MODIFICAR</a>';
	echo '<a class="btn btn-sm btn-primary col-lg-12 " align="center" href="consultaCursos.php">VOLTAR</a>';
// while ($login = $resultado->fetch_object())
